...
- `controller/relay.go` — PreConsumeBilling 之前插入 TPM 限速检查
- `service/text_quota.go` — PostTextConsumeQuota 中添加限速用量记录调用
...
```bash
git add service/rate_limit.go service/rate_limit_test.go controller/subscription.go
git commit -m "feat(rate-limit): add rate limit parameter validation for plan CRUD"
```
...
func TestCleanupOnSubscriptionExpiry(t *testing.T) {
	// [示意] 需要数据库和 Redis 环境
	// 验证订阅过期时 CleanupRateLimitData 被调用
	// 此处验证调用链路，不验证 Redis 操作本身（已在 T3 覆盖）
	// 通过检查 service 层函数是否正确接收 subscriptionId 来验证
}
```
...
func TestCheckUserCodingPlanRateLimit_Exceeded(t *testing.T) {
	// [示意] 需要 Redis mock
	// Pre-populate Redis with usage data exceeding limit
	// Verify CheckUserCodingPlanRateLimit returns exceeded=true
}
```
...
在 `service/text_quota.go` 的 `PostTextConsumeQuota` 函数中，在 `SettleBilling` 调用之后（约行 375 之后），添加限速用量记录：
...
在 `service/text_quota.go` 的 `PostTextConsumeQuota` 函数中，在限速用量记录之后，添加响应头注入：
